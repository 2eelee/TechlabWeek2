...
// Console Window 그리기
// 매 프레임 호출
void FConsoleWindow::Draw(const char* title, bool* pOpen)
{
    // Console Window 시작
    ...
    // 로그 출력 영역
    if (ImGui::BeginChild("ScrollingRegion", ImVec2(0, -ImGui::GetFrameHeightWithSpacing()),
        ImGuiChildFlags_NavFlattened, ImGuiWindowFlags_HorizontalScrollbar))
    {
        // Copy 버튼을 누르면 출력 로그를 Clipboard로 복사
        if (copyToClipboard)
    ...
    // Enter를 누르면 true 반환
    if (ImGui::InputText("Input", InputBuf, IM_COUNTOF(InputBuf), inputFlags, &TextEditCallbackStub, (void*)this))
    {
        char* s = InputBuf;
...
{
    // 사용자가 입력한 명령어도 Console에 출력
    AddLog("# %s", commandLine);

    HistoryPos = -1;

    // 같은 명령어가 History에 있다면 기존 명령어 삭제
    ...
    {
        for (int i = 0; i < History.Size; i++)
            AddLog("%d: %s", i, History[i]);
    }

    // CLASSIFY
    else if (Stricmp(commandLine, "CLASSIFY") == 0)
        AddLog("CLASSIFY command received.");

    // 없는 명령어
    else
        AddLog("Unknown command: '%s'", commandLine);

    // 새로운 로그가 생겼으므로 다음 프레임에 아래쪽으로 스크롤
    ScrollToBottom = true;
}

// InputText Callback 연결 함수
int FConsoleWindow::TextEditCallbackStub(ImGuiInputTextCallbackData* data)
{
    FConsoleWindow* console = (FConsoleWindow*)data->UserData;
    return console->TextEditCallback(data);
}

// TAB 자동완성 / ↑ ↓ History
int FConsoleWindow::TextEditCallback(ImGuiInputTextCallbackData* data)
{
